...
async function peticion() {
  ...
  dato.types.forEach((pokemones) => console.log(pokemones.type.name));
}
...
async function llamado() {
  ...
  let libro = await fetch(
    "https://openlibrary.org/search.json?q=title%3A+%22i+robot%22"
  );
...
//https://api.nasa.gov/ - https://api.nasa.gov/neo/rest/v1/feed?start_date=START_DATE&end_date=END_DATE&api_key=API_KEY

document.getElementById("btn7").addEventListener("click", asteroide);

async function asteroide() {
  console.log("cargando... Ejercicio 7");
  let data = await fetch(
    "https://api.nasa.gov/neo/rest/v1/feed?start_date=2023-08-09&end_date=2023-08-16&api_key=DEMO_KEY"
  );
  let tierra = await data.json();

  let tierraArray = Object.values(tierra.near_earth_objects);

  let busqueda = tierraArray.forEach((item) => {
    item.filter(x => x.is_potentially_hazardous_asteroid).map((fila) => console.log(fila.name));
